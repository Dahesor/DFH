...
* `$by_entity`: **(只读)** 若本次伤害由`entity_hurt_player`进度触发器触发，则为1 advancement trigger. Run `execute on attacker` to access the attacking entity。可以执行`execute on attacker`获取这个实体。
